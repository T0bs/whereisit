...
- [x] **M14 — Heuristic learns from prior placements.** Adds a third signal to the M9 placement cascade: for each candidate container, compute Sørensen–Dice similarity between the new item's name tokens and each existing child's name tokens, take the max, and scale by 1.5 (Dice tops out around 0.4 on short item names even for a clear single-word match, so scaling brings real matches above `HIGH_CONFIDENCE`). The container's final score is `max(tag_kind_score, scaled_neighbor_score)` — a container can win either by matching tags/kind OR by already holding similar things. `_heuristic_reason` surfaces the winning sibling name so the UI can explain "Suggested because Tool drawer already contains 'Claw hammer'". Sibling names are loaded in one batch query alongside the candidate list.
  - *Considerations:* zero schema change — the existing tree IS the training data. Stopwords (the, a, of, with, …) are stripped before tokenisation so phrases like "the box" / "the bag" don't create false matches. Single-character tokens dropped. Max (not sum) over siblings keeps the metric bounded and prevents popular containers from drowning out specific matches. Calibration: 1 shared significant word among 2-word names = 0.4 raw → 0.6 scaled → just at threshold; full token overlap = 1.0; no overlap = 0.

- [x] **M13 — Bulk add + suggest categories.** Lets the user index things in batches when first cataloguing their home. New schema: `nodes.suggested_parent_id` (nullable FK to nodes, ON DELETE SET NULL) for pending suggestions; new kind `inbox` with a lazily-created top-level "Uncategorized" container as the holding pen. Endpoints: `POST /bulk-add` (creates `kind=item` leaves under Uncategorized, ≤100 names per call); `GET /bulk/state` (one-shot snapshot of inbox id + items + every can_contain category with its path); `POST /ai/suggest-categories` (per-item, reuses the M9 placement cascade with `exclude_ids={inbox}`, writes the top pick to `suggested_parent_id`); `POST /ai/accept-categories` (atomic per row via savepoints — moves the node, clears the suggestion, reports per-row ok/error). DELETE on any `kind=inbox` node is refused. UI: 4th tab **Bulk** — textarea for batch entry, checkbox table of uncategorized items with a category dropdown pre-filled with the suggestion (user can override), one button to "Suggest for selected", one to "Accept selected". TreeView hides the inbox subtree so the main view stays clean.
  - *Considerations:* the inbox itself is never offered as a suggested category; bulk-add accepts up to 100 names at a time with stripped blanks and >255-char names reported in `skipped`; partial acceptance is supported (one bad row doesn't roll back the good ones); same two-gate cloud rule applies to `/ai/suggest-categories`. Web-only by deliberate choice — no CLI / MCP duplication. Suggestion history is single-valued (latest replaces previous); upgrade to a `category_suggestions` table later if multi-rank or history is needed.
